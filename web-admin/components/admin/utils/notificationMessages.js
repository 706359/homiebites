export const notificationMessages = {
  orders: {
    ...
    clearAllSuccess: (count) =>
      `Successfully deleted ${count} order${count !== 1 ? 's' : ''}`,
  ...
  menu: {
    ...
    itemAddError:
      'Failed to add menu item. Please check the details and try again.',
  ...
  gallery: {
    ...
    addError:
      'Failed to add gallery item. Please check the details and try again.',
  ...
  upload: {
    ...
    excelValidationError:
      'Invalid Excel file. Please ensure all required columns are present.',
...
export const getNotificationMessage = (category, key, ...args) => {
  ...
  if (!categoryMessages) {
    return (
      notificationMessages.general[key] || notificationMessages.general.error
    );
  }
