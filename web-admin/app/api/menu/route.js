...
export async function PUT(request) {
  try {
    ...
    if (!Array.isArray(data)) {
      return Response.json(
        { success: false, error: 'Menu must be an array' },
        { status: 400 }
      );
    }

    console.log(
      '[Menu API] Full payload received:',
      ...
      return Response.json(
        {
          success: false,
          error:
            'Cannot save empty menu. Please add at least one menu item with a category.',
        },
        { status: 400 }
      );
    }

    const categoriesWithItems = data.filter(
      (cat) => cat.items && Array.isArray(cat.items) && cat.items.length > 0
    );
      ...
      return Response.json(
        {
          success: false,
          error:
            'Cannot save menu with no items. Please ensure each category has at least one menu item.',
        },
        { status: 400 }
      );
    }
    ...
    for (const category of data) {
      if (!category.category) {
        console.error(
          '[Menu API] Error: Category missing name field:',
          category
        );
        return Response.json(
          {
      ...
      if (!Array.isArray(category.items)) {
        console.error(
          '[Menu API] Error: Category items is not an array:',
          category
        );
        return Response.json(
          {
    ...
    if (existingDoc) {
      ...
      if (totalItems > 0) {
        ...
        console.log(
          '[Menu API] Created new menu document with',
          totalItems,
          'items'
        );
      } else {
        console.error(
          '[Menu API] Cannot create new menu document with no items'
        );
        return Response.json(
          {
            success: false,
            error:
              'Cannot create menu with no items. Please add at least one menu item before saving.',
          },
          { status: 400 }
        );
      }
    ...
    const savedCategoriesWithItems =
      savedDoc?.data?.filter(
        (cat) => cat.items && Array.isArray(cat.items) && cat.items.length > 0
      ) || [];
    const savedTotalItems =
      savedDoc?.data?.reduce((sum, cat) => sum + (cat.items?.length || 0), 0) ||
      0;

    console.log(
      '[Menu API] Full saved document:',
    ...
    if (!savedDoc || !savedDoc.data) {
      console.error(
        '[Menu API] Saved document verification failed - no data field'
      );
      throw new Error(
        'Menu data was not saved correctly - verification failed'
      );
    }

    if (!Array.isArray(savedDoc.data)) {
      console.error(
        '[Menu API] Saved data is not an array:',
        typeof savedDoc.data
      );
