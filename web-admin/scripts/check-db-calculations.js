...
async function checkCalculations() {
  try {
    ...
    const missingTotalOrders = orders.filter(o => 
      (!o.totalAmount || o.totalAmount === 0) && 
      (!o.quantity || !o.unitPrice)
    );
    console.log(`Orders missing both totalAmount and qty/price: ${missingTotalOrders.length}\n`);
    ...
    Object.entries(statusCounts).slice(0, 10).forEach(([status, count]) => {
