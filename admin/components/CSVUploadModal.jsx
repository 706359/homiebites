// CSV Upload Modal Component - Following FULL_DASHBOARD_PLAN.md structure
...
const CSVUploadModal = ({
  ...
  const handleFileSelect = (selectedFile) => {
    ...
    if (selectedFile.type === 'text/csv' || selectedFile.name.toLowerCase().endsWith('.csv')) {
      ...
      // For Excel files, try to estimate or show a message
      // We can't parse Excel files client-side without a library, so show estimated count
      // The actual count will be determined by the server
      setPreviewData({ 
        headers: [], 
        rows: [], 
        totalRows: null, // null means "unknown - will be determined by server"
        isExcel: true 
      });
    }
  };
  ...
  const validateCSVData = (headers, rows) => {
    const errors = [];
    const requiredColumns = ['Date', 'Delivery Address', 'Quantity', 'Unit Price', 'Total Amount', 'Mode', 'Status', 'Payment Mode'];
  ...
  const handleUpload = async () => {
    ...
    const totalRows = previewData?.totalRows !== null && previewData?.totalRows !== undefined 
  ...
  const handleDownloadTemplate = () => {
    const template = `Date,Delivery Address,Quantity,Unit Price,Total Amount,Mode,Status,Payment Mode
15-Jan-25,A3-1206,2,100,200,Lunch,Paid,Online
15-Jan-25,B2-405,3,100,300,Dinner,Pending,Cash
14-Jan-25,C1-789,2,100,200,Lunch,Paid,UPI`;
        ...
        <div className="modal-body">
          ...
          {!file && (
            ...
            >
              ...
              <p style={{ color: 'var(--admin-text-light)', fontSize: '0.85rem', marginTop: '16px' }}>
              ...
              <div className="dashboard-card" style={{ marginBottom: '24px' }}>
                ...
                {previewData && (
                  <div>
                    ...
                    <div className="orders-table-container" style={{ maxHeight: '300px', overflow: 'auto' }}>
                    ...
                    <p style={{ color: 'var(--admin-text-secondary)', fontSize: '0.85rem', marginTop: '8px' }}>
                      {previewData.isExcel 
                ...
                {/* Upload Options */}
                <div style={{ marginTop: '24px', paddingTop: '24px', borderTop: '2px solid var(--admin-border)' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}>
                      ...
                      <span>
                        ...
                        <br />
                        <span style={{ fontSize: '0.85rem', color: 'var(--admin-text-secondary)', marginLeft: '24px' }}>
                          When checked, orders with matching Order IDs will be updated instead of creating duplicates
                        </span>
                      </span>
                    </label>
                    <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}>
                    ...
                    <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}>
                      <input
                        type="checkbox"
                        checked={uploadOptions.autoGenerateOrderIds}
                        onChange={(e) =>
                          setUploadOptions({ ...uploadOptions, autoGenerateOrderIds: e.target.checked })
                        }
                      />
                      <span>Auto-generate Order IDs (for records without Order ID)</span>
                    </label>
                  </div>
                </div>
              </div>
          ...
          {isUploading && !isProgressMinimized && (
            <div className="dashboard-card" style={{ textAlign: 'center', padding: '48px', position: 'relative' }}>
              <button
              ...
              <h3 style={{ marginBottom: '24px' }}>Uploading Orders...</h3>
          ...
          {uploadStatus === 'success' && (
            ...
            >
              ...
              ></i>
              <h3 style={{ color: 'var(--admin-success)', marginBottom: '16px' }}>Upload Complete!</h3>
          ...
          {!file && (
            <div className="dashboard-card" style={{ marginTop: '24px' }}>
              ...
              <ul style={{ margin: 0, paddingLeft: '20px', color: 'var(--admin-text-secondary)' }}>
                <li>Columns: Date, Delivery Address, Quantity, Unit Price, Total Amount, Mode, Status, Payment Mode</li>
                <li>Date format: DD-MMM-YY or DD/MM/YYYY</li>
                <li>No empty rows</li>
