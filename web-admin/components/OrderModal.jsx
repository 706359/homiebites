...
const OrderModal = ({ isOpen, onClose }) => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        const items = response.data
          .filter((item) => item.isActive !== false && item.price && item.price > 0)
          .map((item) => ({
            id: item._id || item.id,
            name: item.name,
            price: item.price,
  ...
  const getAvailableItemsForDropdown = () => {
    ...
    const availableItems = galleryItems.filter((item) => !allAddedIds.has(item.id));
  ...
  const loadAllAddresses = async () => {
    try {
      ...
      const cacheTimestamp = localStorage.getItem('homiebites_addresses_cache_time');
      ...
      if (response.success && response.data) {
        ...
        localStorage.setItem('homiebites_all_addresses', JSON.stringify(response.data));
        localStorage.setItem('homiebites_addresses_cache_time', Date.now().toString());
    ...
    for (let i = addressStartIndex; i < parts.length; i++) {
      ...
      // If part is all letters
        ...
        if (foundAddressPart) {
          ...
          if (part.length <= 5 && /^(Block|Flat|House|Building|Apt|Unit|No|#)$/i.test(part)) {
            addressParts.push(part);
          }
  ...
  const getAddressSuggestions = (query) => {
    ...
    if (cachedAddresses.length > 0) {
      const filtered = getCachedAddressSuggestions(searchQuery, cachedAddresses);
  ...
  const fetchAddressSuggestionsFromAPI = async (query) => {
    ...
    try {
      ...
      const response = await api.getAddressSuggestions(searchQuery, abortController.signal);
      ...
      if (response.success && response.data) {
        ...
        // Extract only address part (remove names) for display
        const addressesOnly = response.data.map(addr => extractAddressOnly(addr));
        setAddressSuggestions(addressesOnly);
  ...
  const validateOrderForm = () => {
    ...
    if (!customerName || !customerName.trim()) {
      errors.customerName = t('order.errors.nameRequired') || 'Customer name is required';
    } else if (customerName.trim().length < 2) {
      errors.customerName = t('order.errors.nameMinLength') || 'Name must be at least 2 characters';
    }
    ...
    if (!displayDate || !displayDate.trim()) {
      errors.deliveryDate = t('order.errors.dateRequired') || 'Delivery date is required';
    } else {
      const parsedDate = parseDateFromDDMMYYYY(displayDate);
      if (!parsedDate) {
        errors.deliveryDate = t('order.errors.dateInvalid') || 'Please enter date in DD/MM/YYYY format (e.g., 22/01/2026)';
      } else {
        const selectedDate = new Date(parsedDate);
        const today = new Date();
        today.setHours(0, 0, 0, 0);
        if (isNaN(selectedDate.getTime())) {
          errors.deliveryDate = t('order.errors.dateInvalid') || 'Please enter a valid date';
        } else if (selectedDate < today) {
          errors.deliveryDate = t('order.errors.datePast') || 'Delivery date cannot be in the past';
        }
      }
    }
    ...
    if (totalQuantity === 0) {
      errors.items = t('order.errors.itemsRequired') || 'Please select at least one item';
    }

    // Minimum order amount validation
    const totalAmount = getTotalPrice();
    if (deliveryMode === 'home' && totalAmount < 70) {
      errors.minimumOrder = t('order.errors.minimumOrderHome') || 'Minimum order of ₹70 required for home delivery. Please select pickup option for orders below ₹70.';
    }
      ...
      <div className="order-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="order-modal-content">
              ...
              {showAddressSuggestions && addressSuggestions.length > 0 && (
                <div className="address-suggestions-dropdown">
                  {addressSuggestions.map((addressDisplay, index) => {
                    ...
                    const fullAddress = addressSuggestionsFull[index] || addressDisplay;
                    return (
                      <button
                        key={index}
                        type="button"
                        className="address-suggestion-item"
                        onClick={() => handleAddressSuggestionSelect(fullAddress)}
          ...
          <div className="form-group">
            ...
            {validationErrors.deliveryMode && (
              <span className="error-message">{validationErrors.deliveryMode}</span>
            )}
            {validationErrors.minimumOrder && (
              <span className="error-message">{validationErrors.minimumOrder}</span>
            )}
          </div>
          ...
          {deliveryMode === 'home' && (
            <div className="form-group">
              <label>{t('order.deliveryTime') || 'Delivery Time Slot'} *</label>
              <select
                value={deliveryTime}
                onChange={(e) => {
                  if (!kitchenStatus.isOpen) return;
              ...
              {validationErrors.deliveryTime && (
                <span className="error-message">{validationErrors.deliveryTime}</span>
              )}
            </div>
          )}
        ...
        <div className="order-form-section">
          <div className="order-items-header">
            ...
            {!loading && galleryItems.length > 0 && getAvailableItemsForDropdown().length > 0 && (
                  ...
                  <div className="add-more-dropdown">
                    {getAvailableItemsForDropdown().map((item) => (
                      <button
                        ...
                        type="button"
                      >
                        <span className="add-more-item-name">{item.name}</span>
                        <span className="add-more-item-price">₹{item.price}</span>
                      </button>
                    ))}
                  </div>
                )}
          ...
          {loading ? (
            ...
            <div className="order-items-empty">
              {getDefaultItems().length === 0
                ...
                ? t('order.selectTimeFirst') || 'Please select a delivery time slot to see available items.'
                : t('order.noItemsForTimeSlot') || 'No items available for the selected time slot. Please try another time slot or add items manually.'}
            </div>
          ) : (
            <>
              {deliveryMode === 'home' && deliveryTime && (
            ...
            {getTotalPrice() > 0 && (
              <>
                ...
                <div className="order-summary-row">
                  ...
                  <span>
                    {deliveryMode === 'pickup' ? (
                      <span className="free-delivery">{t('order.freeDelivery') || 'FREE'}</span>
                    ) : getDeliveryCharge() === 0 ? (
                      <span className="free-delivery">{t('order.freeDelivery') || 'FREE'}</span>
                    ) : (
                      `₹${getDeliveryCharge()}`
                ...
                {deliveryMode === 'home' && getTotalPrice() > 0 && getTotalPrice() < 100 && (
                  <div className="order-minimum-notice">
                    {(t('order.addMoreForFreeDelivery') || 'Add ₹{amount} more for free delivery').replace(
                      '{amount}',
                      String(100 - getTotalPrice())
                    )}
                  </div>
                )}
                {deliveryMode === 'home' && getTotalPrice() > 0 && getTotalPrice() < 70 && (
                  <div className="order-minimum-warning">
                    {t('order.minimumOrderWarning') || 'Minimum order of ₹70 required for home delivery. Please select pickup option for orders below ₹70.'}
                  </div>
                )}
                <div className="order-summary-total">
                  <strong>
                    {t('order.grandTotal') || 'Grand Total'}: ₹{getGrandTotal()}
                  </strong>
                </div>
              </>
            )}
