  ...
  const renderStars = (rating) => {
    return (
      <div className='review-stars'>
        {[1, 2, 3, 4, 5].map((star) => (
          <i key={star} className={`fa-solid fa-star ${star <= rating ? 'active' : ''}`} />
        ))}
      </div>
    );
  };
        ...
        {reviews.length > 0 ? (
          ...
          <div className='no-reviews'>
            ...
            <h3 className='no-reviews-title'>
              {t('reviews.noReviewsTitle') || 'Be the First to Share Your Experience!'}
            </h3>
            <p className='no-reviews-message'>
              {t('reviews.noReviews') || 'No reviews yet. Be the first to review!'}
            </p>
            <button className='btn btn-primary btn-large' onClick={() => setShowReviewForm(true)}>
              <i className='fa-solid fa-pen'></i> {t('reviews.writeReview') || 'Write a Review'}
            </button>
          </div>
        )}
