...
**Rule:** *Never use inline style, only use classes.*
...
**Rule:** *Never use `!important` without actual need. Prefer specificity and selector hierarchy.*
...
**Rule:** *All borders must be exactly 1px. No 1.5px, 2px, 3px, 4px, 5px.*
...
**Rule:** *Use CSS variables for colors and buttons. No hardcoded hex for colors.*
...
**Rule:** *Explicitly applies to `border`, `border-top`, `border-bottom`, `border-left`, `border-right`, `border-width`.*  
...
**Rule:** *`styles/globals.css` — ONLY shared classes used in both web and admin. No admin-specific styles.*
...
**Rule:** *Use variables only for colors and buttons. Use direct `px` for spacing, sizing, borders (except color), etc.*
...
**Rule:** *Create only very important .md files.*
...
**Rule:** *Always check for duplicate classes before creating new; don’t create new CSS files without confirmation.*
