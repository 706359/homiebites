...
export const useOrderManagement = ({
    ...
    async (orderData, isEdit = false) => {
      ...
      if (token && (isEdit ? orderData._id || orderData.id : true)) {
        try {
          ...
          if (isEdit) {
            const response = await api.updateOrder(orderData._id || orderData.id, apiOrder);
            if (response.success) {
              await loadOrders();
          ...
          console.warn(`Failed to ${isEdit ? 'update' : 'save'} order via API:`, apiError.message);
          showNotification(`Order saved locally (API unavailable)`, 'warning');
        }
      }

      // Fallback to localStorage - use temporary ID if new order (backend will generate on sync)
      const tempId = orderId || `TEMP-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`;
      const orderDataForStorage = {
        ...
        created_at: isEdit ? orderData.created_at : new Date().toISOString(),
      ...
      if (isEdit) {
        updatedOrders = orders.map((order) =>
          order.id === orderData.id || order.orderId === orderId ? orderDataForStorage : order
        );
        showNotification('Order updated successfully!', 'success');
      } else {
        updatedOrders = [...orders, orderDataForStorage];
        showNotification('Order added successfully!', 'success');
      }

      setOrders(updatedOrders);
      try {
        setOrdersHook && setOrdersHook(updatedOrders);
      } catch (e) {}
      localStorage.setItem('homiebites_orders', JSON.stringify(updatedOrders));
      if (lastSyncedOrdersRef) {
        lastSyncedOrdersRef.current = updatedOrders;
      }

      return true;
    },
    [orders, setOrders, setOrdersHook, lastSyncedOrdersRef, showNotification, loadOrders]
  );
  ...
  const deleteOrder = useCallback(
    async (orderId) => {
      ...
      const updatedOrders = orders.filter(
        (order) =>
          order.orderId !== orderId && order.id !== orderId && order._id !== orderId
      );
      setOrders(updatedOrders);
      try {
        setOrdersHook && setOrdersHook(updatedOrders);
  ...
  const updateOrderStatus = useCallback(
    ...
    [orders, setOrders, setOrdersHook, lastSyncedOrdersRef]
  );
