...
import { getCurrentUser, logoutUser, updateProfile, saveAddress, getUserOrders } from '../../lib/userAuth';
        ...
        <div className="account-container">
          <div className="account-sidebar">
            ...
            <div className="account-tabs">
              ...
              <button
                className="account-tab logout-btn"
                      ...
                      {user.addresses.map((addr) => (
                        <div key={addr.id} className={`address-card ${addr.isDefault ? 'default' : ''}`}>
                          ...
                          <div className="address-info">
                            ...
                            <p>{addr.phone}</p>
                            <p>{addr.address}</p>
                            {addr.landmark && <p>{t('account.landmark')}: {addr.landmark}</p>}
                            {addr.pincode && <p>{t('account.pincode')}: {addr.pincode}</p>}
                          </div>
                        </div>
                      ))}
                    </div>
              ...
              <div className="account-section">
                ...
                {orders.length > 0 ? (
                    ...
                    {orders.map((order) => (
                      <div key={order.id} className="order-card">
                        <div className="order-header">
                          <strong>{t('common.order')} #{order.sNo || order.id}</strong>
                        ...
                        <div className="order-total">
                          <strong>{t('common.total')}: ₹{order.total || order.totalAmount}</strong>
                        </div>
                        {order.deliveryAddress && (
                          <div className="order-address">
                            <p><strong>{t('order.deliveryAddress')}:</strong></p>
                  ...
                  <div className="no-data">
                    ...
                    <p><Link href="/menu">{t('menu.viewMenu')}</Link></p>
                  </div>
                )}
              </div>
            )}
